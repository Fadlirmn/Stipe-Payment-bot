...
## Konteks
Staff kesulitan melakukan verifikasi ulang pada tautan Stripe Checkout yang gagal setelah deadline tugas terlewati atau kuota harian habis. Selain itu, sinkronisasi Sheets -> DB tidak memperbarui data penugasan (`assigned_to`) dan status final (seperti perubahan dari `HTTP_ERR` ke `OK`).
