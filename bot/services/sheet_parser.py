...
async def copy_active_key_to_sheet(
    api_key: str,
    email: str,
    username: str,
    tokens: int | str,
    ...
    """
    Mengirim request POST ke Apps Script untuk menyalin API Key aktif ke tab spreadsheet spesifik (Active Keys).
    """
    import os
    if not APPS_SCRIPT_URL:
        logger.warning("[SheetParser] APPS_SCRIPT_URL belum diset, skip copy_active_key_to_sheet")
        return False

    target_gid = os.getenv("GOOGLE_SHEET_TARGET_GID", "324874213")
    payload = {
        "action": "copyActiveKey",
        ...
        "check_keys_timestamp": datetime.now(TZ).strftime("%Y-%m-%d %H:%M:%S")
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT, follow_redirects=True) as client:
            ...
            resp.raise_for_status()
            res_json = resp.json()
            logger.info(f"[SheetParser] Copy Active Key Sheet: api_key={api_key[:6]}...{api_key[-6:] if len(api_key) > 12 else ''}, resp={res_json}")
            return res_json.get("status") == "copied"
    except Exception as exc:
        logger.error(f"[SheetParser] Gagal memanggil Apps Script copyActiveKey: {exc}")
        return False
